...
{
    this -> spriteSheetTexture = image;     ///spritesheet containing alphabets
    ...
    float posX = position.x ;   //positioning the word according to he length
...
void Word::changeWord(std::string text)
{
    ...
    for(int i=0; i<wordLen; i++)
    {
        chars[i] = Character(spriteSheetTexture,posX,position.y, (int)text[i]); //make charaters and store that in dynamic array
        width += 64;
    }
}
